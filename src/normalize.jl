...
const RE_NUM = r"\d+"
const BLANK = ' '
const PUNCTUACTION_BLANK = Set(string(PUNCTUACTION, BLANK))
const EMOJIS = Set([l[1] for l in readlines(joinpath(@__DIR__, "emojis.txt"))])

# SKIP_WORDS = set(["…", "..", "...", "...."])


function _string_preprocessing(config::TextConfig, text)
    if config.lc
        text = lowercase(text)
    end
...
"""
    normalize(config::TextConfig, text::AbstractString)::Vector{Char}

Normalizes a given text using the specified transformations of `config`

"""
function normalize(config::TextConfig, text::AbstractString)::Vector{Char}
    text = _string_preprocessing(config, text)
    L = Char[BLANK]
    prev = BLANK
