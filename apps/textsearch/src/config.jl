...
# Stopwords are DETECTED once, on the first part, and reused by every later part -- a stopword is a
# property of the language rather than of the batch. Verified on Spanish: across five
# 400k-paragraph batches from two different shards, the first batch's 31 tokens are a superset of
# every other batch's set and equal their union. It also makes merging exact, since parts that all
# remove the same set leave no token with partial counts.
#
# If your corpus is in ARBITRARY order, or its beginning is topically narrow, the first batch may
# not be representative. The remedy is a larger `batch_size` above (equivalently, fewer parts): a
# bigger first batch is a better sample of the corpus, and detection is paid once no matter how
# many parts follow. Doubling the batch costs about 1.7x rather than 2x, so this is cheap.
